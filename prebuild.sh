...
# immortalwrt 把 luci/packages feed 锁在旧提交，appfilter 停在 6.x；这里追加固定到 OAF v7.0.1 的 feed
OAF_COMMIT='b88fcb082597486a816187ec1e02812082161d5e'
if ! grep -q '^src-git oaf ' feeds.conf.default; then
    echo "src-git oaf https://github.com/destan19/OpenAppFilter.git^${OAF_COMMIT}" >> feeds.conf.default
fi
./scripts/feeds clean
...
git clone --branch openwrt-24.10 --depth 1 https://github.com/tofuliang/openwrt-package;
cp -fr openwrt-package/luci-app-control-webrestriction luci-app-control-webrestriction;
cp -fr openwrt-package/luci-app-accesscontrol luci-app-accesscontrol;
